...
public class BillRecordDML {
    ...
    // Constructor
    public BillRecordDML(Connection connection) {
     ...
     */
    public int recordPayment(int billId, int tenantId, double paymentAmount,
                             String paymentMethod, String referenceNumber,
                             int landlordId, String notes) {
        String sql = "INSERT INTO BillRecords (bill_id, tenant_id, payment_amount, " +
                "payment_method, reference_number, landlord_id, notes) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?)";
        int recordId = -1;
        ...
        try {
            ...
            try (PreparedStatement stmt = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                ...
                if (rowsAffected > 0) {
                    ...
                    if (billDML.markBillAsPaid(String.valueOf(billId))) {
    ...
    public ResultSet getPaymentRecordsByTenantId(int tenantId) {
        String sql = "SELECT br.*, b.bill_type, b.amount as bill_amount, " +
    ...
    public Map<String, Object> generateMonthlyIncomeReport(int year, int month) {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            report.put("billTypeBreakdown", getBillTypeBreakdown(startDate, endDate));
    ...
    /**
     * Get breakdown of income by bill type for a date range
     *
     * @param startDate Start date of the range
     * @param endDate End date of the range
     * @return List of maps containing bill type and amount
     */
    private List<Map<String, Object>> getBillTypeBreakdown(LocalDate startDate, LocalDate endDate) {
        List<Map<String, Object>> breakdown = new ArrayList<>();

        String sql = "SELECT b.bill_type, SUM(br.payment_amount) as total_income, " +
                "COUNT(br.record_id) as payment_count " +
                "FROM BillRecords br " +
                "JOIN Bills b ON br.bill_id = b.bill_id " +
                "WHERE DATE(br.payment_date) BETWEEN ? AND ? " +
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    typeData.put("billType", rs.getString("bill_type"));
                    typeData.put("income", rs.getDouble("total_income"));
                    typeData.put("count", rs.getInt("payment_count"));
                    breakdown.add(typeData);
                }
            }
        } catch (SQLException e) {
            System.err.println("Error getting bill type breakdown: " + e.getMessage());
        }
    ...
    public Map<String, Object> generateTenantPaymentHistoryReport(int tenantId, LocalDate startDate, LocalDate endDate) {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(tenantSql)) {
            ...
            String billTypeSql = "SELECT b.bill_type, COUNT(*) as count, SUM(br.payment_amount) as total " +
                    "FROM BillRecords br " +
                    "JOIN Bills b ON br.bill_id = b.bill_id " +
            ...
            List<Map<String, Object>> billTypeBreakdown = new ArrayList<>();
            try (PreparedStatement billTypeStmt = connection.prepareStatement(billTypeSql)) {
                billTypeStmt.setInt(1, tenantId);
                billTypeStmt.setDate(2, Date.valueOf(startDate));
                billTypeStmt.setDate(3, Date.valueOf(endDate));

                try (ResultSet rs = billTypeStmt.executeQuery()) {
                    while (rs.next()) {
                        Map<String, Object> typeData = new HashMap<>();
                        typeData.put("billType", rs.getString("bill_type"));
                        typeData.put("count", rs.getInt("count"));
                        typeData.put("total", rs.getDouble("total"));
                        billTypeBreakdown.add(typeData);
                    }
                }
            }
            report.put("billTypeBreakdown", billTypeBreakdown);
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    payment.put("billType", rs.getString("bill_type"));
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    receipt.put("billType", rs.getString("bill_type"));
    ...
    public List<Map<String, Object>> generateLatePaymentsReport(int days) {
        ...
        String sql = "SELECT b.bill_id, b.bill_type, b.amount, b.due_date, " +
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    latePayment.put("billType", rs.getString("bill_type"));
    ...
    public ResultSet searchPaymentRecords(Map<String, Object> criteria) {
        ...
        sqlBuilder.append("SELECT br.*, b.bill_type, b.amount as bill_amount, ");
        sqlBuilder.append("r.room_number, f.floor_number, bg.building_name, ");
        sqlBuilder.append("u.name as landlord_name, u2.name as tenant_name ");
        ...
        if (!criteria.isEmpty()) {
            ...
            if (criteria.containsKey("billType")) {
                sqlBuilder.append("AND b.bill_type = ? ");
                params.add(criteria.get("billType"));
            }
    ...
    /**
     * Get the most commonly used payment method for a tenant
     *
     * @param tenantId ID of the tenant
     * @return String representing the most common payment method, or null if no payments
     */
