...
public class App {
    public static void main(String[] args) {
        // Create a building
        Building building = new Building("Sunrise Apartments");
        System.out.println("Created building: " + building.getBuildingName());

        // Add floors
        building.addFloor(1);
        building.addFloor(2);
        building.displayBuildingDetails(); // Should show 2 floors

        // Get a floor and add rooms
        Floor floor1 = building.searchFloor(1);
        if (floor1 != null) {
            floor1.addRoom(new Room("101", "Small", false, 50, 20));
            floor1.addRoom(new Room("102", "Medium", true, 100, 50));
        }
        building.displayBuildingDetails(); // Should show 2 rooms on Floor 1

        // Test room occupancy
        Room room101 = floor1.searchRoom("101");
        if (room101 != null) {
            room101.markAsOccupied();
            room101.updateUsage(120, 60); // Increase counter
            room101.displayRoomBilling();
        }

        // Test removal logic
        floor1.removeRoom("101"); // Should NOT remove because it's occupied
        room101.markAsVacant(); // Make it vacant
        floor1.removeRoom("101"); // Now it should be removable

        // Attempt to remove a floor with rooms
        building.removeFloor(1); // Should fail because Floor 1 still has Room 102
        floor1.removeRoom("102"); // Remove last room
        building.removeFloor(1); // Now it should succeed

        // Final building state
        building.displayBuildingDetails(); // Should show only Floor 2 remaining
    }
}